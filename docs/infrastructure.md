...
**После копирования тот же скрипт проверяет результат** и валит сборку, если
хоть один `/_next/static/*`, на который ссылается пререндеренный HTML, до
вывода не доехал. Проверка появилась не от любви к проверкам: ровно этот
сбой один раз доехал до продакшена. Отсутствовал **один** CSS-чанк — с ним
отвалились все CSS-модули сразу, и наружу это выглядело не как сломанная
статика, а как «кнопка входа не нажимается» и «меню не открывается»:
разметка и обработчики на месте, модалка в DOM появляется, но без
`position: fixed` и `z-index` её просто не видно. Сборка при этом зелёная,
healthcheck проходит, деплой считается успешным.

Типичная причина — несвежий `.next`: HTML остался от прошлой сборки, а хеши
чанков перегенерировались. Лечится удалением `.next` и кэша сборки на
платформе. Проверить живой сайт можно так — в консоли браузера на любой
странице:

```js
const html = await (await fetch(location.href, { cache: "reload" })).text();
const refs = [...new Set([...html.matchAll(/\/_next\/static\/[^"'\\)\s]+/g)].map(m => m[0]))];
for (const r of refs) {
  const res = await fetch(r, { cache: "no-store" });
  if (!res.ok) console.error(res.status, r);
}
```
